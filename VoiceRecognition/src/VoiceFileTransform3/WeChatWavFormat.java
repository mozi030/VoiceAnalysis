...
//		System.out.println("dataSize: " + dataSize);
		...
		wavFormat.samples = new int[dataSize / 2];
		for (int i = 0 ; i < dataSize; i += 2) {
			int currentSample = 
					(wavFormat.data[i + 1] << 8)  & 0x0000ff00 | 
					(wavFormat.data[i] << 0)  & 0x000000ff;
			
			if (currentSample >= 32768) {
				currentSample = currentSample - 65536;
			}
			
			wavFormat.samples[i / 2] = currentSample;
		}
	...
	static public byte[] transferWavFormatToBytes(WeChatWavFormat wavFormat) {
		wavFormat.dataSize = wavFormat.samples.length * 2;
		wavFormat.data = new byte[wavFormat.samples.length * 2];
		for (int i = 0; i < wavFormat.samples.length; i++) {
			byte[] temp = ByteBuffer.allocate(4).putInt(wavFormat.samples[i]).array();
			temp = reverseBytes(temp);
			System.arraycopy(temp, 0, wavFormat.data, 2 * i, 2);
		}
		...
		temp = ByteBuffer.allocate(4).putInt(wavFormat.formatType).array();
		temp = reverseBytes(temp);
		System.arraycopy(temp, 0, bytes, 20, 2);

		temp = ByteBuffer.allocate(4).putInt(wavFormat.channelNum).array();
		temp = reverseBytes(temp);
		...
		temp = ByteBuffer.allocate(4).putInt(wavFormat.check2).array();
		temp = reverseBytes(temp);
		System.arraycopy(temp, 0, bytes, 32, 2);

		temp = ByteBuffer.allocate(4).putInt(wavFormat.BitsPerSample).array();
		temp = reverseBytes(temp);
		System.arraycopy(temp, 0, bytes, 34, 2);
		
		System.arraycopy(wavFormat.LIST, 0, bytes, 36, 4);
		
		temp = ByteBuffer.allocate(4).putInt(wavFormat.listSize).array();
		temp = reverseBytes(temp);
		System.arraycopy(temp, 0, bytes, 40, 4);
		
		System.arraycopy(wavFormat.listData, 0, bytes, 44, wavFormat.listSize);
		
		System.arraycopy(wavFormat.DATA, 0, bytes, 70, 4);

		temp = ByteBuffer.allocate(4).putInt(wavFormat.dataSize).array();
		temp = reverseBytes(temp);
		System.arraycopy(temp, 0, bytes, 74, 4);
		
		System.arraycopy(wavFormat.data, 0, bytes, 78, wavFormat.dataSize);
